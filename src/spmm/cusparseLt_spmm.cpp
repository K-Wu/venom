/*
 * Copyright (C) 2023 Roberto Lopez Castro (roberto.lopez.castro@udc.es). All
 * Rights Reserved.
 *
...
CusparseLt_Spmm<T, T2>::CusparseLt_Spmm(Dataset<T, T2> &d, cudaDataType_t S,
                                        cusparseComputeType C_spmm)
    : Spmm_CNN<T, T2>(d) {
  ...
  int64_t batch_strideA =
      A_height * lda + 128;  // TODO: 128? As a Buffer?? Or alignment?
  ...
  // SET NUM BATCHES
  CHECK_CUSPARSELT(
      cusparseLtMatDescSetAttribute(&handle, &matA, CUSPARSELT_MAT_NUM_BATCHES,
                                    &num_batches, sizeof(num_batches)))
  CHECK_CUSPARSELT(
      cusparseLtMatDescSetAttribute(&handle, &matB, CUSPARSELT_MAT_NUM_BATCHES,
                                    &num_batches, sizeof(num_batches)))
  CHECK_CUSPARSELT(
      cusparseLtMatDescSetAttribute(&handle, &matC, CUSPARSELT_MAT_NUM_BATCHES,
                                    &num_batches, sizeof(num_batches)))
  //--------------------------------------------------------------------------
  // SET BATCH STRIDE
  ...
  // the matrix A
  CHECK_CUSPARSELT(
      cusparseLtMatDescSetAttribute(&handle, &matA, CUSPARSELT_MAT_BATCH_STRIDE,
                                    &batch_strideA, sizeof(batch_strideA)))
  CHECK_CUSPARSELT(
      cusparseLtMatDescSetAttribute(&handle, &matB, CUSPARSELT_MAT_BATCH_STRIDE,
                                    &batch_strideB, sizeof(batch_strideB)))
  CHECK_CUSPARSELT(
      cusparseLtMatDescSetAttribute(&handle, &matC, CUSPARSELT_MAT_BATCH_STRIDE,
                                    &batch_strideC, sizeof(batch_strideC)))
  ...
  size_t workspace_size, compressed_size, compressed_buffer_size;
...
CusparseLt_Spmm<T, T2>::~CusparseLt_Spmm() {
  ...
  CUDA_CHECK(cudaFree(dA_compressedBuffer))
...
template <class T, class T2>
float CusparseLt_Spmm<T, T2>::spmm(int times, int bm_, int bn_, int bk_,
                                   int wm_, int wn_, int wk_, int mm_, int mn_,
                                   int mk_, int nstage_) {
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  time = cuTime(times, cusparseLtMatmul, &handle, &plan, &alpha, dA_compressed,
                dB, &beta, dC, dD, d_workspace, streams, num_streams);
